...
export module KeyCombinationsModule {
   ...
   */
  export function convertToKeyCombination(e: KeyboardEvent): string {
    let payload: string = '';
   ...
   */
  export function listActiveModifiers(e: KeyboardEvent): string[] {
    let payload: string[] = [];

    if (e.ctrlKey) payload.push('Control');
    if (e.shiftKey) payload.push('Shift');
    if (e.altKey) payload.push('Alt');
    if (e.metaKey) payload.push('Meta');
   ...
   */
  export function isModifier(e: KeyboardEvent): boolean {
